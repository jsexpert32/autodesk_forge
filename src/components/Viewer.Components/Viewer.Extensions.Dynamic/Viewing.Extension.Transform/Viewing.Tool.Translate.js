...
export default class TranslateTool extends EventsEmitter {
  ...
  constructor(viewer) {
    ...
    this.onAggregateSelectionChanged = this.onAggregateSelectionChanged.bind(
      this
    );
  ...
  createTransformMesh() {
    ...
    this._viewer.impl
      .matman()
      .addMaterial('transform-tool-material', material, true);
      ...
      case 'SELECTION_MODE_TRANSFORM':
        if (this._selection && this.pointerDown) {
          ...
          if (this._selection.model.selector) {
            this._selection.model.selector.setSelection(this._dbIds);
          }
  ...
  setSelection(selection) {
    ...
    if (this.fullTransform) {
      ...
      var fragCount = this._selection.model.getFragmentList().fragments
        .fragId2dbId.length;
