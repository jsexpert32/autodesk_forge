...
import ExtensionBase from '../../Viewer.Extensions/Viewer.ExtensionBase';
...
class TransformExtension extends ExtensionBase {
  ...
  load() {
    ...
    if (typeof this.parentControl === 'string') {
      ...
      this.parentControl = viewerToolbar.getControl(this.parentControl);
    }
  ...
  restoreState(viewerState, immediate) {
    ...
    if (viewerState.transforms) {
      ...
      this.animateTransform(viewerState, easingFunc, period).then(() => {
        this.currentExplodeScale = viewerState.explodeScale;
      });

      this.transformedFragIdMap = Object.assign({}, viewerState.transforms);
  ...
  restoreTransform(targetState) {
    const currentFragIds = Object.keys(this.transformedFragIdMap);

    const targetFragIds = Object.keys(targetState.transforms);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const fragProxyTasks = fullFragIds.map(fragId => {
        ...
        fragProxy.step = {
          ...
          dQx:
            (targetTransform.quaternion._x - fragProxy.quaternion._x) / period,
          dQy:
            (targetTransform.quaternion._y - fragProxy.quaternion._y) / period,
          dQz:
            (targetTransform.quaternion._z - fragProxy.quaternion._z) / period,
          dQw:
            (targetTransform.quaternion._w - fragProxy.quaternion._w) / period
        };
        ...
        return {
          ...
          ease: t => {
            ...
            fragProxy.position.x =
              eased * targetPos.x + (1 - eased) * initPos.x;
            fragProxy.position.y =
              eased * targetPos.y + (1 - eased) * initPos.y;
            fragProxy.position.z =
              eased * targetPos.z + (1 - eased) * initPos.z;
      ...
      if (targetScale != scale) {
        ...
        animationTasks.push({
          step: dt => {
            ...
            ViewerToolkit.selectiveExplode(viewer, scale, fullFragIds);
          },
          ...
          ease: t => {
            ...
            ViewerToolkit.selectiveExplode(viewer, easedScale, fullFragIds);
          },

          finalStep: () => {
            ViewerToolkit.selectiveExplode(viewer, targetScale, fullFragIds);
