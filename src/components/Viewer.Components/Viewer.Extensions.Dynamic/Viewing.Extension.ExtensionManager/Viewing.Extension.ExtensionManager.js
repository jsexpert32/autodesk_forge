...
import './Viewing.Extension.ExtensionManager.css';
...
class ExtensionManager extends MultiModelExtensionBase {
    ...
    this.reactOpts = {
      pushRenderExtension: extension => {
        return new Promise(async resolve => {
          ...
          this.react
            .setState({
              renderExtensions: [...state.renderExtensions, extension]
            })
            .then(async () => {
              resolve();
      ...
      popRenderExtension: extensionId => {
        ...
        const renderExtensions = state.renderExtensions.filter(ext => {
  ...
  load() {
    ...
    const extensionsByName = sortBy(this.options.extensions || [], ext => {
      return ext.name;
    });
      ...
      .then(async () => {
        ...
        const loadExts = extensions.filter(extension => {
          if (this.options.useStorage) {
            ...
            extension.enabled =
              extension.enabled || storageExtensions.includes(extension.id);
          }
  ...
  loadDynamicExtension(extension) {
    return new Promise((resolve, reject) => {
      ...
      const options = Object.assign({}, this.options, extension.options, {
      ...
      // with the viewer API
      if (extension.native) {
        this.viewer.loadExtension(extension.id, options).then(
          extInstance => {
        ...
        this.viewer.loadDynamicExtension(extension.id, options).then(
          extInstance => {
            extension.loading = false;
            extension.enabled = true;
  ...
  async onExtensionItemClicked(extension) {
    ...
    if (extension.enabled) {
      ...
      const { extensions, renderExtensions } = this.react.getState();

      extension.enabled = false;

      const renderExts = renderExtensions.filter(ext => {
      ...
      const extInstance = await this.loadDynamicExtension(extension);
  ...
  render() {
    ...
    const nbExt = renderExtensions.length + (this.options.visible ? 1 : 0);
    ...
    return <PaneManager orientation="horizontal">{panes}</PaneManager>;
  }
}
