...
function TreeOnDemand(treeDelegate, root, parentContainer, options) {
  ...
  var nodeIndexToNodeCssTable = new Uint16Array(
    treeDelegate.getTreeNodeCount()
  );
  ...
  var createTables = function(node) {
    ...
    nodeIndexToNodeCssTable[nodeIndex] = treeDelegate.isTreeNodeGroup(node)
      ? 1
      : 2;
...
proto.scrollTo = function(node) {
  ...
  for (
    var i = this.options.excludeRoot ? 1 : 0;
    i < expandedHeightStackCount;
    ++i
  ) {
...
proto.iterate = function(node, callback) {
  ...
  this.treeDelegate.forEachChild(
    node,
    function(child) {
...
function redraw(tree, inmediate) {
  ...
  var createTreeElements = function(node) {
    ...
    var elementHeight =
      tree.nodeDepth === -1
        ? 0
        : tree.treeDelegate.getTreeNodeClientHeight(node);
    ...
    if (
      elementBottom < scroller.top ||
      elementTop > scroller.top + scroller.height
    ) {
    ...
    if (scroller.empty) {
      ...
      scroller.style.top =
        scroller.top -
        (elementHeight - (container.height - scroller.top)) +
        'px';
    }
    ...
    if (elementHeight > 0) {
      ...
      var offset = tree.treeDelegate.getTreeNodeDepthOffset(
        node,
        tree.nodeDepth
      );
  ...
  var createElementTree = function() {
    ...
    scroller.empty = true;
    (scroller.top = Math.max(
      0,
      containerParent.scrollTop - SCROLL_SAFE_PADDING
    )),
      tree.rootContainer.height;
    ...
    var height = Math.max(
      0,
      Math.min(container.height, scroller.top + scroller.height) - scroller.top
    );
...
function onPanelScroll(event) {
  ...
  if (
    scroller.top + scroller.height <
      container.parentNode.scrollTop + parentSize.height ||
...
function onElementMouseOut(event) {
  ...
  if (
    getNodeIdFromElement(this, event.target) !==
    getNodeIdFromElement(this, element)
  ) {
    this.treeDelegate.onTreeNodeHover(this, -1, event);
    event.stopPropagation();
    event.preventDefault();
...
function ModelStructurePanel(parentContainer, id, title, options) {
  ...
  options = options || {};
  if (!options.heightAdjustment) options.heightAdjustment = 40;
...
ModelStructurePanel.prototype = Object.create(
...
ModelStructurePanel.prototype.createUI = function() {
  ...
  if (!instanceTree /*|| this.uiCreated*/) return;
  ...
  var createDelegate = function() {
    ...
    delegate.getTreeNodeId = function(node) {
      if (typeof node == 'object') {
        Autodesk.Viewing.Private.logger.warn(
          'Object used instead of dbId. Fix it.'
        );
        return node.dbId;
      } else return node;
    };
    ...
    delegate.getTreeNodeIndex = function(node) {
      ...
      } else return that.instanceTree.nodeAccess.dbIdToIndex[node];
    };
    ...
    delegate.getTreeNodeLabel = function(node) {
      ...
      if (dbId == -1e10)
        // Replace Object -10000000000 with Object 0
        res = 'Object 0';
  ...
  var treeOptions = {
    ...
    startCollapsed:
      !!(that.options && that.options.startCollapsed) && !skipRoot,
    localize: true
  };
  that.tree = new TreeOnDemand(
    delegate,
    rootId,
    that.scrollContainer,
    treeOptions
  );
...
ModelStructurePanel.prototype.getContentSize = function() {
  ...
  if (tree) {
    ...
    if (treeContainer) {
      return {
        height: treeContainer.clientHeight + this.options.heightAdjustment + 35,
...
function MultiModelStructurePanel(viewer, title, options) {
  ...
  options = options || {};
  options.excludeRoot =
    options.excludeRoot !== undefined ? options.excludeRoot : true;
  ...
  ModelStructurePanel.call(
    this,
    viewer.container,
    viewer.container.id + 'MultiModelStructurePanel',
    title,
    options
  );

  this.clickConfig =
    options && options.docStructureConfig
...
MultiModelStructurePanel.prototype = Object.create(
  ModelStructurePanel.prototype
);
MultiModelStructurePanel.prototype.constructor = MultiModelStructurePanel;
Autodesk.Viewing.Extensions.ViewerPanelMixin.call(
  MultiModelStructurePanel.prototype
);
...
MultiModelStructurePanel.prototype.initialize = function() {
  ...
  that.addEventListener(that.viewer, Autodesk.Viewing.ISOLATE_EVENT, function(
    event
  ) {
    that.setIsolation(event.nodeIdArray.slice());
  });
  that.addEventListener(that.viewer, Autodesk.Viewing.HIDE_EVENT, function(
    event
  ) {
    that.setHidden(event.nodeIdArray.slice(), true);
  });
  that.addEventListener(that.viewer, Autodesk.Viewing.SHOW_EVENT, function(
    event
  ) {
...
MultiModelStructurePanel.prototype.createUI = function() {
  ...
  var treeNodesContainer = this.container.querySelector(
    '#MultiModelStructurePanel-scroll-container'
  );
  treeNodesContainer.addEventListener(
    'contextmenu',
    function(event) {
...
MultiModelStructurePanel.prototype.setHidden = function(nodes, hidden) {
  if (!this.uiCreated) this.createUI();
...
MultiModelStructurePanel.prototype.setIsolation = function(nodes) {
  ...
  var tree = this.tree;
  tree &&
    tree.iterate(this.rootId, function(node) {
      tree.removeClass(node, 'dim', false);
      tree.removeClass(node, 'visible', false);
      return true;
    });
...
MultiModelStructurePanel.prototype.initSearchBox = function() {
  ...
  function doSearch() {
    ...
    if (searchbox.value.length === 0) {
      ...
      if (trackAdpFirstSearch) {
        Autodesk.Viewing.Private.logger.track({
  ...
  searchbox.addEventListener('change', function(e) {
    // immediate: press enter, lose focus
    clearTimeout(timeout);
    doSearch();
