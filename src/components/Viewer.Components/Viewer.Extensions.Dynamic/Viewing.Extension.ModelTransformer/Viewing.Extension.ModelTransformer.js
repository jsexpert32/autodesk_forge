...
*/
  ...
  load() {
    ...
    this.viewer
      .loadDynamicExtension('Viewing.Extension.Transform', options)
      .then(transformExtension => {
        transformExtension.setFullTransform(fullTransform);

        transformExtension.on('selection', this.onTransformSelection);

        transformExtension.on('transform', this.onTransform);

        transformExtension.on('deactivate', this.onDeactivate);
  ...
  async onTransform(data) {
    ...
    if (fullTransform) {
      ...
      const transform = this.getFragmentTransform(data.fragIds[0]);
  ...
  onSelection(event) {
    ...
    const selection = event.selections.length ? event.selections[0] : null;
    ...
    if (!fullTransform) {
      if (selection) {
        const transform = this.getFragmentTransform(selection.fragIdsArray[0]);
  ...
  /////////////////////////////////////////////////////////
  getTransform() {
    const { fullTransform, model, selection } = this.react.getState();
  ...
  /////////////////////////////////////////////////////////
  translate() {
    const { transformExtension, translate } = this.react.getState();
  ...
  /////////////////////////////////////////////////////////
  rotate() {
    const { transformExtension, rotate } = this.react.getState();
  ...
  renderControls() {
    ...
    const disabled = state.fullTransform ? !model : !selection;
    ...
    return (
      <div className="controls">
        ...
        {this.options.showFullModelTransform && (
          <div className="row">
            <label>Full Model Transform:</label>
            <Switch
              onChange={checked => {
