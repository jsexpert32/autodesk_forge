(function() {
  ...
  var coreNamespace = AutodeskNamespace(
    'Autodesk.Viewing.Extensions.Markups.Core'
  );
  var utilsNamespace = AutodeskNamespace(
    'Autodesk.Viewing.Extensions.Markups.Core.Utils'
  );
  ...
  MarkupsPanel.prototype = Object.create(
  ...
  proto.render = function() {
    ...
    this.btnUndo = this.scrollContainer.querySelector(
      '[data-annotation-action="undo"]'
    );
    this.btnRedo = this.scrollContainer.querySelector(
      '[data-annotation-action="redo"]'
    );
    this.ddAnnotationFont = this.scrollContainer.querySelector(
      '[data-markup-style="font-family"]'
    );

    //get all the style inputs and create a style controller for each one
    this.styleControls = {};
    var styleElements = this.scrollContainer.querySelectorAll(
      '[data-markup-style]'
    );
    toList(styleElements).map(
      function(element) {
  ...
  var clearButtonGroup = function(groupingElements) {
    ...
    else containers = toList(groupingElements);
  ...
  proto.selectAnnotationButton = function(element, options) {
    ...
    clearButtonGroup(
      this.container.querySelectorAll('.viewer-actions, .markup-buttons')
    );
  ...
  var activateApplicableControls = function() {
    ...
    var defaultValues = utilsNamespace.getStyleDefaultValues(
      currentStyles,
      markupsCore
    );
    ...
    for (var activeStyle in defaultValues) {
      ...
      var styleElement = this.container.querySelector(
        '[data-markup-style="' + activeStyle + '"]'
      );
      var styleControl;
      ...
      styleControl.value =
        ...
        styleSettings.values[styleSettings['default']].value;
    }

    //the markup styles are grouped together by the style which they modify(fill, stroke, font)
    //iterate through each group
    toList(this.container.querySelectorAll('.markup-properties > div')).map(
      function(div) {
  ...
  proto.switchEditMode = function(type) {
    ...
    var editMode = new Autodesk.Viewing.Extensions.Markups.Core[className](
      this.markupsCore
    );
  ...
  proto.handleUIEvents = function() {
    ...
    on('click', '[data-viewer-tool]', function(event) {
      ...
      //clear the button group this button belongs to
      clearButtonGroup(
        self.container.querySelectorAll('.viewer-actions, .markup-buttons')
      );
  ...
  proto.getContentSize = function getContentSize() {
    return {
      height:
        this.container.querySelector('.markups-panel-content').offsetHeight +
        55,
  ...
  proto.handleCoreEvents = function() {
    this.markupsCore.addEventListener(
      coreNamespace.EVENT_HISTORY_CHANGED,
      function() {
    ...
    this.markupsCore.addEventListener(
      coreNamespace.EVENT_MARKUP_SELECTED,
      function(event) {
        var target = event.target;
        ...
        this.container
          .querySelector('[data-annotation-action="delete"]')
          .classList.add('active');
      }.bind(this)
    );

    this.markupsCore.addEventListener(
      coreNamespace.EVENT_MARKUP_DESELECT,
      function(event) {
        this.container
          .querySelector('[data-annotation-action="delete"]')
