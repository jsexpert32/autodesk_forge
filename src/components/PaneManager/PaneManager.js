...
class PaneManager extends React.Component {
  ...
  flatMapChildren(children, fn) {
    return Array.prototype.concat.apply([], React.Children.map(children, fn));
  }
  ...
  cloneChildren(children, state) {
    const nextChildren = this.flatMapChildren(children, (child, idx) => {
      const childId =
        state.children.length > 2 * idx
          ? state.children[2 * idx].props.id
          : null;

      const childState = childId ? state[childId] : {};
      ...
      const splitter = (
        <ReflexSplitter style={splitterStyle} propagate={showSplitter} />
      );
